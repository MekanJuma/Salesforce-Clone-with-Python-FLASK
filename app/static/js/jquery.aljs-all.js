...
(function ($) {
    ...
    var Datepicker = function (el, options) {
        ...
        for (var i = 0; i < this.settings.dayLabels.length; i++) {
            ...
            labeledTableMarkup = labeledTableMarkup
                .replace(fullLabelExp, thisLabel.full.toString())
                .replace(abbrLabelExp, thisLabel.abbr.toString());
        }

        this.$datepickerEl = $(
            datepickerMenuMarkup.replace(
                /{{assetsLocation}}/g,
    ...
    Datepicker.prototype = {
        ...
        initInteractivity: function () {
            ...
            var openDatepicker = function (e) {
                ...
                if (
                    ...
                    $([$el, $datepickerEl, $elEndDate, $el.prev("svg")]).each(
                        function () {
        ...
        closeDatepicker: function (e) {
            ...
            if (
                $target.closest(self.$el.parent()).length === 0 &&
                $selectedInput
            ) {
                var $selectedDatepickerEl = $selectedInput
                    .closest(".slds-form-element")
                    .find(".slds-datepicker");
        ...
        fillMonth: function () {
            ...
            monthArray.forEach(function (rows) {
                ...
                rows.data.forEach(function (col, colIndex) {
                    var $dayCol = $(
                        '<td data-aljs-date="' + col.dateValue + '">'
                    ).appendTo($weekRow);
                    ...
                    if (!col.isCurrentMonth) {
                        $dayCol
                            .addClass("slds-disabled-text")
                            .attr("aria-disabled", "true");
                    }

                    if (
                        col.isSelected ||
                        col.isSelectedEndDate ||
                        col.isSelectedMulti
                    ) {
                        // ||
                        ...
                        $dayCol
                            .addClass(
                                isMultiSelect
                                    ? "slds-is-selected-multi slds-is-selected"
                                    : "slds-is-selected"
                            )
            ...
            $monthTableBody.append(
                todayLinkMarkup
                    ...
                    .replace(
                        /{{todayLabel}}/g,
                        this.settings.todayLabel.toString()
                    )
            );
            ...
            this.$datepickerEl
                .find("#aljs-month")
                .text(this.settings.monthLabels[this.viewedMonth].full);
        ...
        initYearDropdown: function () {
            ...
            if ($yearSelect.length > 0) {
                ...
                $yearSelect = $(
                    '<select class="slds-select select picklist__label">'
                ).appendTo($yearDropdown);

                var currentYear = moment().year();

                for (
                    var i = currentYear - this.settings.numYearsBefore;
                    i <= currentYear + this.settings.numYearsAfter;
                    i++
                ) {
                    var $yearOption = $(
        ...
        getMonthArray: function () {
            ...
            var selectedFullDate = this.selectedFullDate;
            var selectedEndDate = this.selectedEndDate;
            var selectedDate = selectedFullDate
                ? selectedFullDate.date()
                : null;
            var selectedMonth = selectedFullDate
                ? selectedFullDate.month()
                : null;
            var selectedYear = selectedFullDate
                ? selectedFullDate.year()
                : null;
            ...
            var numDaysInMonth = this.getNumDaysInMonth(
                viewedYear,
                viewedMonth
            );
            var numDaysInPrevMonth = this.getNumDaysInMonth(
                viewedYear,
                previousMonth
            );
            var numDaysInNextMonth = this.getNumDaysInMonth(
                viewedYear,
                nextMonth
            );
            ...
            // Fill previous month
            for (
                var i = numDaysInPrevMonth - (firstDayOfMonth - 1);
                i <= numDaysInPrevMonth;
                i++
            ) {
                var iDate = moment(
                    new Date(
                        previousMonth === 11 ? viewedYear - 1 : viewedYear,
                        previousMonth,
                        i
                    )
                );
            ...
            for (var i = 1; i <= numDaysInMonth; i++) {
                ...
                allDays.push({
                    ...
                    isSelected:
                        selectedFullDate &&
                        iDate.isSame(selectedFullDate, "day"),
                    isSelectedEndDate:
                        selectedEndDate && iDate.isSame(selectedEndDate, "day"),
                    isSelectedMulti:
                        selectedFullDate &&
                        selectedEndDate &&
                        (iDate.isBetween(selectedFullDate, selectedEndDate) ||
                            iDate.isSame(selectedFullDate, "day") ||
                            iDate.isSame(selectedEndDate, "day")),
                    isToday: iDate.isSame(moment(), "day"),
                });
            }
            ...
            allDays.forEach(function (day, index, allDays) {
                if (index % 7 === 0) {
                    var hasMultiRowSelection =
                        index >= 7 &&
                        allDays[index - 1].isSelectedMulti &&
                        day.isSelectedMulti;
            ...
            if (calendarRows[calendarRows.length - 1].data.length < 7) {
                var iDate = moment(
                    new Date(
                        nextMonth === 0 ? viewedYear + 1 : viewedYear,
                        nextMonth,
                        i
                    )
                );
                var numColsToFill =
                    7 - calendarRows[calendarRows.length - 1].data.length;
        ...
        setSelectedFullDate: function (selectedFullDate) {
            ...
            if (
                ...
                typeof selectedFullDate !== "undefined"
            ) {
                ...
                if (
                    ...
                    // Addresses bug where Safari does not clear out visible placeholders on first value update
                    this.$el.val(selectedFullDate.format(this.settings.format));
                }
            } else {
                this.$el.val("");
            ...
            if (
                (!oldDate && selectedFullDate != "") ||
                (typeof oldDate === "object" &&
                    !oldDate.isSame(selectedFullDate, "day"))
            ) {
        ...
        setSelectedEndDate: function (selectedEndDate) {
            ...
            if (
                ...
                this.$elEndDate.val(
                    selectedEndDate.format(this.settings.format)
                );
            ...
            if (
                (!oldDate && selectedEndDate != "") ||
                (typeof oldDate === "object" &&
                    !oldDate.isSame(selectedEndDate, "day"))
            ) {
                this.settings.onChange(this);
            }
        },
        ...
        processBlur: function (e) {
            if (e) {
                ...
                if (
                    self.$elEndDate &&
                    self.$elEndDate.length > 0 &&
                    self.$elEndDate[0] === self.$selectedInput[0]
                ) {
                    // Check if current input is for an end date
                    var selectedEndDate = self.$elEndDate.val();
                    var momentSelectedEndDate = moment(
                        selectedEndDate,
                    ...
                    var momentSelectedFullDate = moment(
                        selectedFullDate,
                        self.settings.format
                    );
        ...
        clickPrev: function (e) {
            ...
            if (self.viewedMonth === 0) {
                ...
                if (
                    self.viewedYear ===
                    currentYear - self.settings.numYearsBefore
                ) {
                    // Allow looping
                    self.viewedYear = currentYear + self.settings.numYearsAfter;
                    self.$datepickerEl
                        .find(".slds-select option:selected")
                        .prop("selected", false);
                    self.$datepickerEl
                        .find(".slds-select option")
                        .last()
        ...
        clickNext: function (e) {
            ...
            if (self.viewedMonth === 11) {
                ...
                if (
                    self.viewedYear ===
                    currentYear + self.settings.numYearsAfter
                ) {
                    // Allow looping
                    self.viewedYear =
                        currentYear - self.settings.numYearsBefore;
        ...
        getDate: function (format) {
            return format
                ? this.selectedFullDate.format(format)
    ...
    $.fn.datepicker = function (options, val) {
        ...
        if (this.length > 1) {
            throw new Error(
                "Usage only allowed for the collection of a single element (" +
                    option +
                    " function)"
            );
        } else {
...
(function ($) {
    $.fn.iconGroup = function (options) {
        ...
        if (typeof options !== "string") {
            ...
            iconGroupObj.defaultIcon =
                iconGroupObj.settings.defaultButtonId === ""
                    ? iconGroupObj.buttons.eq(0)
                    : "#" + iconGroupObj.settings.defaultButtonId;
            ...
            iconGroupObj.buttons.click(function () {
                ...
                if (
                    iconGroupObj.target.hasClass(isSelected) &&
                    (iconGroupObj.settings.type === "toggle" ||
                        iconGroupObj.settings.type === "switch")
                ) {
                    deselect(iconGroupObj.target);
                    iconGroupObj.settings.onChange();
                } else if (
                    iconGroupObj.target.hasClass(notSelected) &&
                    iconGroupObj.settings.type === "toggle"
                ) {
                    ...
                    iconGroupObj.target.hasClass(notSelected) &&
                    (iconGroupObj.settings.type === "switch" ||
                        iconGroupObj.settings.type === "sort")
                ) {
                    iconGroupObj.buttons.each(function () {
                        if (
                            $(this) !== iconGroupObj.target &&
                            $(this).hasClass(isSelected)
                        )
                            deselect($(this));
                    });
            ...
            throw new Error(
                "This plugin can only be run with a selector, or with a command"
            );
        }
    };
})(jQuery);
...
(function ($) {
    ...
    var Lookup = function (el, options) {
        ...
        if (this.isSingle) {
            this.$singleSelect = $(selectContainerMarkup).insertBefore(
                this.$el
            );
        } else {
    ...
    Lookup.prototype = {
                ...
                selectedResults.forEach(function (result) {
                    var $pill = $(
                        conditionalPillMarkup
                            .replace(
                                "{{objectIconUrl}}",
                                self.settings.objectIconUrl
                            )
                            .replace(
                                "{{objectIconClass}}",
                                self.settings.objectIconClass
                            )
                            .replace(
                                "{{hasIcon}}",
                                self.settings.objectIconUrl !== ""
                            ...
                            .replace(
                                "{{assetsLocation}}",
                                self.settings.assetsLocation
                            )
                            .replace(
                                /{{objectLabel}}/g,
                                self.settings.objectLabel
                            )
        ...
        setSingleSelect: function (selectedResultLabel) {
            ...
            if (selectedResultLabel) {
                ...
                this.$singleSelect.one(
                    "click",
                    "button",
                    this,
                    this.clearSingleSelect
                ); //'a, button', this, this.clearSingleSelect);
            } else {
                this.$singleSelect
                    .addClass("slds-hide")
        ...
        getSearchTermResults: function (searchTerm) {
            ...
            if (this.settings.items.length > 0) {
                this.searchResults = this.settings.items.filter(function (
                    item
                ) {
                    return (
                        item.label
                            .toLowerCase()
                            .match(searchTerm.toLowerCase()) !== null
                    );
                });
        ...
        renderSearchResults: function () {
            ...
            if (!self.isStringEmpty(searchTerm) && searchTerm.length) {
                if (self.settings.showSearch) {
                    ...
                    $resultsListContainer.prepend(
                        searchMarkup
                            ...
                            .replace(
                                "{{objectPluralLabel}}",
                                self.settings.objectPluralLabel
                            )
                            .replace(
                                "{{assetsLocation}}",
                                $.aljs.assetsLocation
                ...
                if (self.settings.recentLabel) {
                    $resultsListContainer.before(
                        recentMarkup.replace(
                            "{{recentLabel}}",
                            self.settings.recentLabel
                        )
                    );
                }
            }
            ...
            this.searchResults.forEach(function (result) {
                ...
                if (self.isSingle) {
                    ...
                    var selectedResultsIds = self.selectedResults.map(function (
                        result
                    ) {
                        return result.id;
                    });
            ...
            if (this.settings.onClickNew) {
                var $newItem = $resultsListContainer.append(
                    newItemMarkup
            ...
            var shouldShowSearchContainer =
                (this.searchResults.length > 0 &&
                    this.$el
                        .closest(".slds-form-element")
                        .find(".slds-lookup__list > li").length > 0) ||
        ...
        handleBlur: function (e) {
            ...
            if (
                $(e.relatedTarget).closest(".slds-lookup.slds-is-open")
                    .length === 0 &&
        ...
        selectResult: function (selectedId) {
            var selectedResultArray = this.searchResults.filter(function (
                result
            ) {
            ...
            if (this.isSingle) {
                this.selectedResult =
                    selectedResultArray.length > 0
                        ? selectedResultArray[0]
                        : null;
...
(function ($) {
    ...
    function initModals() {
        aljsScope = $.aljs.scoped
            ? typeof $.aljs.scopingClass === "string"
                ? "." + $.aljs.scopingClass
                : ".slds-scope"
            : aljsBodyTag;
    ...
    function showModal(obj, args) {
        ...
        if (modalId === undefined)
            console.error('No "data-aljs-show" attribute has been set');
        else {
    ...
    $.fn.modal = function (args, options) {
        var modalObj = {};
        modalObj.$el = this;
        modalObj.hasSelector =
            args && args.hasOwnProperty("selector") ? true : false;
        ...
        if (args !== null && typeof args === "string") {
            ...
            function dismissModal() {
                modalObj.$el.modal("dismiss", settings).unbind("click");
            ...
            switch (args) {
                case "show":
                    ...
                    $(".slds-modal")
                        .removeClass("slds-fade-in-open slds-show")
                        .addClass("slds-hide")
                        .attr("aria-hidden", "true")
                        .attr("tabindex", -1);

                    $(".slds-backdrop").remove(); // Remove any existing backdrops
                    $(".aljs-modal-container").append(
                    ...
                    setTimeout(function () {
                        // Ensure elements are displayed and rendered before adding classes
                        ...
                        var handleTransitionEnd = function () {
                            ...
                            isShowing = false;
                            $(aljsBodyTag)
                                .unbind("keyup", processKeyup)
                                .bind("keyup", processKeyup);

                            dismissModalElement.click(function (e) {
                            ...
                            if (settings.backdropDismiss) {
                                ...
                                modalElements.click(function (e) {
                                    e.stopPropagation();
                                });
                            }
                        };
                        ...
                        backdrop
                            .one("transitionend", handleTransitionEnd)
        ...
        } else if (modalObj.hasSelector && this.length === 1) {
            // If allowing for selector to trigger modals post-init
...
(function ($) {
    ...
    var multiSelect = function (el, options) {
        this.$el = $(el);
        this.$selectedContainer = this.$el
            .find('[data-aljs-multi-select="selected"]')
            .find("ul");
        this.$unselectedContainer = this.$el
            .find('[data-aljs-multi-select="unselected"]')
            .find("ul");
    ...
    multiSelect.prototype = {
        ...
        init: function () {
            ...
            this.$el
                .find('[data-aljs-multi-select="unselect"]')
                .on("click", this, this.unselectItem);
            this.$el
                .find('[data-aljs-multi-select="select"]')
                .on("click", this, this.selectItem);
            this.$el
                .find('[data-aljs-multi-select="move-up"]')
                .on("click", this, this.moveItemUp);
            this.$el
                .find('[data-aljs-multi-select="move-down"]')
                .on("click", this, this.moveItemDown);
        },
        renderUnselectedItems: function () {
            ...
            this.unselectedItems.forEach(function (item) {
                self.$unselectedContainer.append(
                    self.createPicklistDomItem(item)
                );
            });
        ...
        selectItem: function (e) {
            ...
            if (self.itemToSelect) {
                var item = self.$unselectedContainer
                    .find("#" + self.itemToSelect.id)
                    .removeClass("slds-is-selected")
                    .attr("aria-selected", "false")
                    .appendTo(self.$selectedContainer);

                self.unselectedItems.splice(
                    self.unselectedItems.indexOf(self.itemToSelect),
                    1
                );
        ...
        unselectItem: function (e) {
            ...
            if (!self.itemToUnselect) {
                return;
            }

            var item = self.$selectedContainer
                .find("#" + self.itemToUnselect.id)
                .removeClass("slds-is-selected")
                .attr("aria-selected", "false")
                .appendTo(self.$unselectedContainer);

            self.selectedItems.splice(
                self.selectedItems.indexOf(self.itemToUnselect),
                1
            );
        ...
        moveItemUp: function (e) {
            ...
            if (itemIndex > 0) {
                ...
                var $itemToMove = self.$selectedContainer.find(
                    "#" + self.itemToUnselect.id
                );
        ...
        destroy: function () {
            ...
            this.$el
                .find('[data-aljs-multi-select="unselect"]')
                .off("click", this.unselectItem);
            this.$el
                .find('[data-aljs-multi-select="select"]')
                .off("click", this.selectItem);
            this.$el
                .find('[data-aljs-multi-select="move-up"]')
                .off("click", this.moveItemUp);
            this.$el
                .find('[data-aljs-multi-select="move-down"]')
                .off("click", this.moveItemDown);

            // Clear selected and unselected items
            this.selectedItems = [];
            this.unselectedItems = [];
            this.$selectedContainer.empty();
            this.$unselectedContainer.empty();

            // Remove multi-select data from the element
            this.$el.removeData("aljs-multi-select");
        },
        createPicklistDomItem: function (item) {
            //var $picklistItem =
            return $(
                picklistItemMarkup
                    .replace("{{optionId}}", item.id)
    ...
    $.fn.multiSelect = function (options) {
        ...
        this.each(function () {
            ...
            if (typeof options === "string") {
                if (options === "destroy" && data) {
                    data.destroy();
                    return;
                }
                internalReturn = data[options](
                    multiSelectArguments[1],
                    multiSelectArguments[2]
                );
            } else if (!data) {
                var multiSelectData = new multiSelect(this, settings);
                $this.data("aljs-multi-select", (data = multiSelectData));
        ...
        if (
            internalReturn === undefined ||
            internalReturn instanceof multiSelect
        ) {
            return this;
        }
...
(function ($) {
    $.fn.notification = function (options) {
        ...
        if (this.length === 1 && typeof options !== "string") {
            ...
            return this.on(
                "click",
                '[data-aljs-dismiss="notification"]',
                function (e) {
            ...
            return this.each(function () {
                ...
                if (!$node.hasClass("slds-notify")) {
                    throw new Error(
                        "This method can only be run on a notification with the slds-notify class on it"
                    );
                } else {
                    if (
                        options === "show" ||
                        (options === "toggle" && $node.hasClass("slds-hide"))
                    ) {
...
(function ($) {
    ...
    Picklist.prototype = {
        ...
        bindTrigger: function () {
            ...
            this.obj.$choices = $(".slds-dropdown__list > li > span", $el).prop(
                "tabindex",
                1
            );

            this.$el
                .on("keyup", self, self.processKeypress)
                .find(".slds-lookup__search-input + .slds-button")
                .css("pointer-events", "none");

            this.obj.$trigger
                .unbind() // Prevent multiple bindings
                .click(function (e) {
                    e.stopPropagation();
                    ...
                    if (self.$el.hasClass("slds-is-open")) {
                        ...
                        if (
                            self.obj.valueId === null ||
                            typeof self.obj.valueId === "undefined"
                        ) {
                            self.focusedIndex = null;
                        } else {
                            self.focusedIndex = self.obj.$dropdown
                                .find("li > span")
                                .index(
                                    self.obj.$dropdown.find(
                                        "#" + self.obj.valueId
                                    )
                                );
                        }
        ...
        processKeypress: function (e) {
            ...
            if (self.$el.hasClass("slds-is-open")) {
                switch (e.which) {
                    ...
                    case 13: // Return
                        if (self.focusedIndex !== null) {
                            var focusedId = self.obj.$choices
                                .eq(self.focusedIndex)
                                .attr("id");
        ...
        setValueAndUpdateDom: function (optionId) {
            ...
            this.obj.$trigger
                .trigger("change.aljs.picklist") // Custom aljs event
                .focus();
    ...
    $.fn.picklist = function (options) {
        ...
        if (
            internalReturn === undefined ||
            internalReturn instanceof Picklist
        ) {
            return this;
        }
...
(function ($) {
    $.fn.pill = function (options) {
        ...
        if (this.length === 1) {
            ...
            throw new Error(
                "This plugin can only be run with a selector targeting one container (e.g., 'body')"
            );
        }
    };
})(jQuery);
...
(function ($) {
    ...
    var showPopover = function (e) {
        var settings = e.data;
        var $target = $(e.target).is($(settings.selector))
            ? $(e.target)
            : $(e.target).closest(settings.selector || '[data-aljs="popover"]');
        ...
        var popoverContent = !isMarkup
            ? htmlEncode($target.attr("data-aljs-title"))
            : $("#" + $target.data("aljs-show")).html();

        if (popoverContent.length > 0) {
            var lineHeightFix = $target.parent().hasClass("slds-button")
                ? ' style="line-height: normal;"'
                : ""; // Adjust line height if popover is inside a button
            var popoverId =
                $target.attr("data-aljs-id") || "aljs-" + new Date().valueOf();
            ...
            var popoverPositioningCSS =
                "overflow: visible; display: block; position: absolute;";
            var modifier =
                settings.modifier != ""
                    ? " slds-popover--" + settings.modifier
                    : "";
            var theme =
                settings.theme != "" ? " slds-theme--" + settings.theme : "";
            var popoverMarkup =
                '<div id="' +
                popoverId +
                '" aria-describedby="' +
                ...
                theme +
                " slds-nubbin--" +
                (popoverNubbins[popoverPosition] || "top") +
                '" style="' +
                popoverPositioningCSS +
                '">' +
            ...
            if ($target.next(".slds-popover").length === 0) {
                var $popoverNode = $.aljs.scoped
                    ? typeof $.aljs.scopingClass === "string"
                        ? $(popoverMarkup).appendTo("." + $.aljs.scopingClass)
                        : $(popoverMarkup).appendTo(".slds-scope")
                    : $(popoverMarkup).appendTo("body");
                var actualWidth = $popoverNode[0].offsetWidth;
                var actualHeight = $popoverNode[0].offsetHeight; // + 15;
                var targetPos = getPosition($target);
                var calculatedOffset = getCalculatedOffset(
                    popoverPosition,
                    targetPos,
                    actualWidth,
                    actualHeight
    ...
    var getCalculatedOffset = function (
        placement,
        pos,
        actualWidth,
        actualHeight
        ...
        switch (placement) {
            case "bottom":
                posObj = {
                    top: pos.top + pos.height + nubbinHeight,
                    left: pos.left + pos.width / 2 - actualWidth / 2,
                };
                break;
            case "top":
                posObj = {
                    top: pos.top - actualHeight - nubbinHeight,
                    left: pos.left + pos.width / 2 - actualWidth / 2,
                };
                break;
            case "left":
                posObj = {
                    ...
                    left: pos.left - actualWidth - nubbinWidth,
                };
                break;
            default: //right
                posObj = {
                    top: pos.top + pos.height / 2 - actualHeight / 2,
                    left: pos.left + pos.width + nubbinWidth,
                };
        }
    ...
    var getViewportAdjustedDelta = function (
        placement,
        pos,
        actualWidth,
        actualHeight
        ...
        if (/right|left/.test(placement)) {
            ...
            var bottomEdgeOffset =
                pos.top - viewportDimensions.scroll + actualHeight;
    ...
    $.fn.popover = function (options) {
        ...
        if (settings.selector && this.length === 1) {
            ...
            return this.each(function () {
                ...
                $(this)
                    .on("mouseenter", thisSettings, showPopover)
    ...
    Tabs.prototype = {
        ...
        initTabs: function () {
            ...
            var children = this.$el.find(
                "> .slds-tabs--default__nav > .slds-tabs--default__item, > .slds-tabs--scoped__nav > .slds-tabs--scoped__item"
            );
            var tabsObj = {
                self: self,
                children: children,
            ...
            // Show first tab
            if (
                this.settings.defaultTabId === "" ||
                $("#" + this.settings.defaultTabId).length === 0
            ) {
